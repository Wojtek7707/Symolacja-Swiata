...
Antylopa::Antylopa(int x, int y)
    : Zwierze('A', 4, 4, x, y) // Inicjalizacja konstruktora klasy bazowej
{
...
void Antylopa::kolizja(Organizm* inny) {
    ...
    {
        std::cout << "Wygrala antylopa" << std::endl;
        if (inny->getZnak() == 'g')
        {
            setSila(3);
        }
    ...
    {
        std::cout << "Antylopa przegrala" << std::endl;
        zabij();
    }
    else
    {
...
Organizm* Antylopa::urodz(int x, int y) {
